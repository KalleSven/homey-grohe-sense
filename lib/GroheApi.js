...
class GroheApi {
  ...
  /**
   * Mark notifications as read/acknowledged in Grohe Cloud.
   */
  async acknowledgeNotifications(locationId, roomId, applianceId, notifications = []) {
    if (!notifications || notifications.length === 0) return {};
    const notifPayload = notifications.map((n) => {
      const id = typeof n === 'object' ? (n.id || n.uuid || n.notification_id) : n;
      return { id, is_read: true, status: 1 };
    });

    try {
      return await this.request(`/locations/${locationId}/rooms/${roomId}/appliances/${applianceId}/notifications`, {
        method: 'POST',
        body: { notifications: notifPayload },
      });
    } catch (err) {
      this.log?.warn?.(`GroheApi: Error acknowledging appliance notifications: ${err.message}`);
      // Fallback: try root /notifications endpoint if appliance-specific endpoint fails
      try {
        ...
        this.log?.warn?.(`GroheApi: Fallback notification acknowledge failed: ${err2.message}`);
      }
    }
    return {};
  }
  ...
  /**
   * Silence / turn off buzzer/alarm on Sense Guard appliance command endpoint
   */
  async silenceAlarmCommand(locationId, roomId, applianceId) {
    try {
      const payload = {
        type: TYPE_SENSE_GUARD,
        command: {
      ...
      return await this.request(`/locations/${locationId}/rooms/${roomId}/appliances/${applianceId}/command`, {
        method: 'POST',
        body: payload,
      });
    } catch (err) {
      this.log?.warn?.(`GroheApi: silenceAlarmCommand failed (hardware/API limitation): ${err.message}`);
    }
    return {};
  }
