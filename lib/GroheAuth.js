...
class GroheAuth {
  ...
  static async loginWithCredentials(email, password) {
    ...
    if (!getRes.ok) {
      throw new Error(`Kunde inte nå Grohes inloggningssida (HTTP ${getRes.status})`);
    }
    ...
    if (!formMatch) {
      throw new Error('Kunde inte identifiera inloggningsformuläret från Grohe Cloud.');
    }
    ...
    if (location && (location.startsWith('ondus://') || location.startsWith('http'))) {
      ...
      if (!tokenRes.ok) {
        throw new Error(`Kunde inte hämta token efter inloggning (HTTP ${tokenRes.status})`);
      }
      const tokenData = await tokenRes.json();
      if (!tokenData.refresh_token) {
        throw new Error('Inget refresh_token returnerades från Grohe Cloud.');
      }
      return tokenData;
    }
    ...
    if (errMatch) {
      ...
      throw new Error(cleanErr || 'Felaktig e-postadress eller lösenord.');
    }

    throw new Error('Inloggningen misslyckades. Kontrollera dina inloggningsuppgifter.');
  }
  ...
  static async resolveTokenInput(input) {
    if (!input || typeof input !== 'string') {
      throw new Error('Vänligen ange en giltig token eller länk.');
    }
    ...
    if (trimmed.startsWith('ondus://') || trimmed.startsWith('https://') || trimmed.startsWith('http://')) {
      ...
      if (!res.ok) {
        throw new Error(`Kunde inte hämta token från länken (HTTP ${res.status}). Kontrollera att länken inte har gått ut.`);
      }
      const data = await res.json();
      if (data.refresh_token) {
        return data.refresh_token;
      }
      throw new Error('Länken innehöll inget giltigt refresh_token.');
    }
